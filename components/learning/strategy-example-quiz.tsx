...
const questions = [
  {
    question: "What is a 'Golden Cross' in the MA crossover strategy?",
    options: [
      "When price crosses above both MAs",
      "When fast MA crosses above slow MA",
      "When fast MA crosses below slow MA",
      "When both MAs are horizontal",
    ],
    correct: 1,
  },
  {
    question: "What should you check with RSI before entering a long trade?",
    options: [
      "RSI should be below 30",
      "RSI should be above 70",
      "RSI should be above 50 but below 70",
      "RSI doesn't matter",
    ],
    correct: 2,
  },
  {
    question: "When using this strategy, what is the recommended risk per trade?",
    options: ["10-20% of account", "5-10% of account", "1-2% of account", "All available capital"],
    correct: 2,
  },
]

interface StrategyExampleQuizProps {
  onBack: () => void
}

export function StrategyExampleQuiz({ onBack }: StrategyExampleQuizProps) {
      ...
      <main className="relative z-10 px-4 sm:px-6 lg:px-16 py-8 lg:py-16">
        <div className="mx-auto max-w-4xl">
          ...
          <Card className="border-2 border-[#5F0BE8]/30 bg-[#1a0f2e]/40 backdrop-blur-xl p-8 rounded-2xl">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-white mb-2">Strategy Example Quiz</h1>
